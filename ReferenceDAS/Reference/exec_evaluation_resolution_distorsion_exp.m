function exec_evaluation_resolution_distorsion_exp(path_image,path_output)
    ...
    %-- Modified on 2017/06/07 by D.N.Rakhmatov (daler@ece.uvic.ca)
    %-- Evaluation: exp.res. phantom vs reconstructed image (RF)
 
    
    addpath(genpath('../Reference/picmus_src'));
    
%     path_scan = ['../Reference/picmus_database/experiments/resolution_distorsion/resolution_distorsion_expe_scan.hdf5'];
    path_phantom = ['../Reference/picmus_database/experiments/resolution_distorsion/resolution_distorsion_expe_phantom.hdf5'];

    
    %-- Parameters
%     category = [1 11 75];

    load(path_image);	%-- MAT structure identifier: "expres_image"
    img = expres_image;
    
    category = img.number_plane_waves;
...
%     image.read_file(path_img);
    ...
    scan = linear_scan();
    scan.x_axis = img.scan.x_axis;
    scan.z_axis = img.scan.z_axis;
    ...
    image.creation_date = '';
    image.scan = scan;
    image.transmit_f_number = img.transmit_f_number;
    image.receive_f_number = img.receive_f_number;
	image.transmit_apodization_window = 'none';
	image.receive_apodization_window = 'none';
    image.number_plane_waves = img.number_plane_waves;
	image.data = img.data;
