...
%-- DAS beamforming: RF data, no windowing, lense shift compensation, "vlong" phantom
...
save('vlong_bfdata_das.mat', 'reshaped_beamformed_data');
...
vlong_image.method = 'DAS beamforming (RF data, no receive apodization)';

save('vlong_image_das.mat', 'vlong_image');
...
%     end;
%     saveas(gcf,sprintf('vlong_das_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('vlong_das_%d.fig',npw));
    saveas(gcf,sprintf('vlong_das_%d',npw),'eps');
    pause(0.5);
end
