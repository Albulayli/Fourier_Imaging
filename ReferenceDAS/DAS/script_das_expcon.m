...
%-- DAS beamforming: RF data, no windowing, lense shift compensation, "expcon" phantom
...
receive_apodization = tools.apodization(rx_aperture_distance,rx_aperture*ones(1,dataset.channels),'none');
...
save('expcon_bfdata_das.mat', 'reshaped_beamformed_data');
...
expcon_image.method = 'DAS beamforming (RF data, no receive apodization)';

save('expcon_image_das.mat', 'expcon_image');
...
%     end;
%     saveas(gcf,sprintf('expcon_das_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('expcon_das_%d.fig',npw));
    saveas(gcf,sprintf('expcon_das_%d',npw),'eps');
    pause(0.5);
end
...
exec_evaluation_contrast_speckle_exp('expcon_image_das.mat','expcon_result_das.txt');
