...
%-- DAS beamforming: RF data, no windowing, lense shift compensation, "expres" phantom
...
save('expres_bfdata_das.mat', 'reshaped_beamformed_data');
...
expres_image.method = 'DAS beamforming (RF data, no receive apodization)';

save('expres_image_das.mat', 'expres_image');
...
%     end;
%     saveas(gcf,sprintf('expres_das_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('expres_das_%d.fig',npw));
    saveas(gcf,sprintf('expres_das_%d',npw),'eps');
    pause(0.5);
end
...
exec_evaluation_resolution_distorsion_exp('expres_image_das.mat','expres_result_das.txt');
