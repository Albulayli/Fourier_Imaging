...
save('vlong_bfdata_pwStolt.mat', 'beamformed_data');
...
vlong_image.method = 'PW Stolt';

save('vlong_image_pwStolt.mat', 'vlong_image');
...
%     end;
%     saveas(gcf,sprintf('vlong_pwStolt_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('vlong_pwStolt_%d.fig',npw));
    saveas(gcf,sprintf('vlong_pwStolt_%d',npw),'eps');
    pause(0.5);
end
