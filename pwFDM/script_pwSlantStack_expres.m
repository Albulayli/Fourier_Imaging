...
save('expres_bfdata_pwSlantStack.mat', 'beamformed_data');
...
expres_image.method = 'PW Slant Stack';

save('expres_image_pwSlantStack.mat', 'expres_image');
...
%     end;
%     saveas(gcf,sprintf('expres_pwSlantStack_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('expres_pwSlantStack_%d.fig',npw));
    saveas(gcf,sprintf('expres_pwSlantStack_%d',npw),'eps');
    pause(0.5);
end
...
exec_evaluation_resolution_distorsion_exp('expres_image_pwSlantStack.mat','expres_result_pwSlantStack.txt');
