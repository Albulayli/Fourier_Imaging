...
%--------------------------------------------------------------------------
%-- Demo using PICMUS benchmark data (IEEE IUS 2016): "vcross" phantom
...
save('vcross_bfdata_pwSlantStack.mat', 'beamformed_data');
...
vcross_image.method = 'PW Slant Stack';

save('vcross_image_pwSlantStack.mat', 'vcross_image');
...
%     end;
%     saveas(gcf,sprintf('vcross_pwSlantStack_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('vcross_pwSlantStack_%d.fig',npw));
    saveas(gcf,sprintf('vcross_pwSlantStack_%d',npw),'eps');
    pause(0.5);
end
