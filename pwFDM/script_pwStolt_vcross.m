...
save('vcross_bfdata_pwStolt.mat', 'beamformed_data');
...
vcross_image.method = 'PW Stolt';

save('vcross_image_pwStolt.mat', 'vcross_image');
...
%     end;
%     saveas(gcf,sprintf('vcross_pwStolt_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('vcross_pwStolt_%d.fig',npw));
    saveas(gcf,sprintf('vcross_pwStolt_%d',npw),'eps');
    pause(0.5);
end
