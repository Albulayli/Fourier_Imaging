...
%--------------------------------------------------------------------------
%-- Demo using PICMUS benchmark data (IEEE IUS 2016): "expcon" phantom
...
load('../Reference/expcon_scan.mat');	%-- MAT structure identifier: "expcon_scan"
rf_scan = expcon_scan;
load('../Reference/expcon.mat');        %-- MAT structure identifier: "expcon"
dataset = expcon;
...
save('expcon_bfdata_pwSlantStack.mat', 'beamformed_data');
...
expcon_image.scan = scan;
expcon_image.number_plane_waves = cellfun('length',pw_indices);
expcon_image.data = resampled_envelope_beamformed_data;
expcon_image.transmit_f_number = 0;
expcon_image.receive_f_number = rx_f_number;
expcon_image.method = 'PW Slant Stack';

save('expcon_image_pwSlantStack.mat', 'expcon_image');
...
%     end;
%     saveas(gcf,sprintf('expcon_pwSlantStack_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('expcon_pwSlantStack_%d.fig',npw));
    saveas(gcf,sprintf('expcon_pwSlantStack_%d',npw),'eps');
    pause(0.5);
end
...
exec_evaluation_contrast_speckle_exp('expcon_image_pwSlantStack.mat','expcon_result_pwSlantStack.txt');
