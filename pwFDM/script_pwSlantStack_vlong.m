...
%--------------------------------------------------------------------------
%-- Demo using PICMUS benchmark data (IEEE IUS 2016): "vlong" phantom
%-- Fourier-domain depth migration (FDM): RF data, modified slant-stack migration, lense shift compensation 
...
load('../Reference/vlong_scan.mat');	%-- MAT structure identifier: "vlong_scan"
rf_scan = vlong_scan;
load('../Reference/vlong.mat');         %-- MAT structure identifier: "vlong"
dataset = vlong;
...
settings.z = z_axis;
settings.x = x_axis;
settings.NtFFT = 2*pow2(nextpow2(length(t_axis)+1));	%-- number of temporal FFT points (1536 -> 4096)
settings.NxFFT = pow2(nextpow2(length(x_axis)+1));      %-- number of spatial FFT points (128 -> 256)
settings.slantMin = -24;                                %-- min. slant angle (deg)
settings.slantMax = 24;                                 %-- max. slant angle (deg)
settings.slantNum = 151;                                %-- number of slants
...
for f=1:length(pw_indices)
    ...
    [BDcomp,~] = pwSlantStack(RFdata,settings);
...
save('vlong_bfdata_pwSlantStack.mat', 'beamformed_data');
...
vlong_image.scan = scan;
vlong_image.number_plane_waves = cellfun('length',pw_indices);
vlong_image.data = resampled_envelope_beamformed_data;
vlong_image.transmit_f_number = 0;
vlong_image.receive_f_number = rx_f_number;
vlong_image.method = 'PW Slant Stack';

save('vlong_image_pwSlantStack.mat', 'vlong_image');
...
%     if npw == 1, title('Slant Stack (Ours), 1 Plane Wave','FontName','Courier','FontSize',10,'FontWeight','bold');
%     else title(sprintf('Slant Stack (Ours), %d Plane Waves',npw),'FontName','Courier','FontSize',10,'FontWeight','bold');
%     end;
%     saveas(gcf,sprintf('vlong_pwSlantStack_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    set(gca,'YDir','reverse');
    npw = length(pw_indices{f});
    if npw == 1, title('Slant Stack (Ours), One PW','FontName','Courier','FontSize',16,'FontWeight','bold');
    else title(sprintf('Slant Stack (Ours), %d PWs',npw),'FontName','Courier','FontSize',16,'FontWeight','bold');
    end;
    saveas(gcf,sprintf('vlong_pwSlantStack_%d.fig',npw));
    saveas(gcf,sprintf('vlong_pwSlantStack_%d',npw),'eps');
    pause(0.5);
end
