...
save('expcon_bfdata_pwStolt.mat', 'beamformed_data');
...
expcon_image.method = 'PW Stolt';

save('expcon_image_pwStolt.mat', 'expcon_image');
...
%     end;
%     saveas(gcf,sprintf('expcon_pwStolt_original_%d.fig',npw));
%     pause(0.5);
% end
...
for f=1:length(pw_indices)    
    ...
    saveas(gcf,sprintf('expcon_pwStolt_%d.fig',npw));
    saveas(gcf,sprintf('expcon_pwStolt_%d',npw),'eps');
    pause(0.5);
end
...
exec_evaluation_contrast_speckle_exp('expcon_image_pwStolt.mat','expcon_result_pwStolt.txt');
